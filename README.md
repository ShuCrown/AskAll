...
- **v1.0.4**（2026-09-02）：支持附件上传（图片/PDF/文档，随问题注入 AI 站点）；新增默认 AI 平台「元宝」；聊天卡片布局（田字格/单列）与 AI 顺序可配置；回答卡片新增「同步」按钮；历史存储解除容量上限、采用分级保留。发布流水线调整为仅构建浏览器扩展，桌面端暂不参与自动打包（详见「桌面端状态说明」）。
